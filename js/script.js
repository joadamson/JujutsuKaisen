...
window.addEventListener('load', () => {
    ...
    if(localStorage.getItem('likedCharaters')){
        return
    }else {
        localStorage.setItem('likedCharacters', JSON.stringify([]));
    }
});
    ...
    const filteredArr = arr.map(item => {
        return `
            <div style="background: url('${item.img}') center / cover no-repeat" class="card">
                <button onclick="showSingle(${item.id})" href="" class="watch">More</button>
...
input.addEventListener('input', e => {
    ...
    const filteredArr = database.filter(({name}) => name.toUpperCase().includes(search));
...
function like(id){
    const cArray = database.filter(item => item.id === id);
    const likedCharacter = cArray[0];
